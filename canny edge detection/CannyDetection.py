...
def linear_convolve(filter, mat, padding=None, strides=[1, 1]):
    ...
    if len(filter_size) == 2 and 1 in filter_size:
        ...
        if filter_size[0] == 1:
            result = convolve(
                filter, mat, [0, 0, padding[0], padding[1]], strides)
        elif filter_size[1] == 1:
            result = convolve(
                filter, mat, [padding[0], padding[1], 0, 0], strides)
...
def DecideAndConnectEdge(g_l, g_t, threshold=None):
    ...
    for i in range(g_l.shape[0]):
        for j in range(g_l.shape[1]):
            isLocalExtreme = True
            eight_neiborhood = g_l[max(
                0, i-1):min(i+2, g_l.shape[0]), max(0, j-1):min(j+2, g_l.shape[1])]
            if eight_neiborhood.shape == (3, 3):
                if g_t[i, j] <= -1:
                    x = 1/g_t[i, j]
                    first = eight_neiborhood[0, 1] + \
                        (eight_neiborhood[0, 1] - eight_neiborhood[0, 0])*x
                    x = -x
                    second = eight_neiborhood[2, 1] + \
                        (eight_neiborhood[2, 2] - eight_neiborhood[2, 1])*x
                    if not (g_l[i, j] > first and g_l[i, j] > second):
                        isLocalExtreme = False
                elif g_t[i, j] >= 1:
                    x = 1 / g_t[i, j]
                    first = eight_neiborhood[0, 1] + \
                        (eight_neiborhood[0, 2] - eight_neiborhood[0, 1]) * x
                    x = -x
                    second = eight_neiborhood[2, 1] + \
                        (eight_neiborhood[2, 1] - eight_neiborhood[2, 0]) * x
                    if not (g_l[i, j] > first and g_l[i, j] > second):
                        isLocalExtreme = False
                elif g_t[i, j] >= 0 and g_t[i, j] < 1:
                    y = g_t[i, j]
                    first = eight_neiborhood[1, 2] + \
                        (eight_neiborhood[0, 2] - eight_neiborhood[1, 2]) * y
                    y = -y
                    second = eight_neiborhood[1, 0] + \
                        (eight_neiborhood[1, 0] - eight_neiborhood[2, 0]) * y
                    if not (g_l[i, j] > first and g_l[i, j] > second):
                        isLocalExtreme = False
                elif g_t[i, j] < 0 and g_t[i, j] > -1:
                    y = g_t[i, j]
                    first = eight_neiborhood[1, 2] + \
                        (eight_neiborhood[1, 2] - eight_neiborhood[2, 2]) * y
                    y = -y
                    second = eight_neiborhood[1, 0] + \
                        (eight_neiborhood[0, 0] - eight_neiborhood[1, 0]) * y
                    if not (g_l[i, j] > first and g_l[i, j] > second):
                        isLocalExtreme = False
            if isLocalExtreme:
